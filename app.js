...
const CHAPTER_SUMMARIES = {
  1: '<div class="mb-2 border-b border-blue-100 pb-1"><span class="text-xs font-bold text-blue-500 uppercase tracking-wider">ENGLISH</span> <span class="text-xs text-blue-400">/</span> <span class="text-xs font-bold text-green-600 uppercase tracking-wider">TIẾNG VIỆT</span></div>'
    ...
    + '<p><span class="text-blue-600 font-medium">• Design:</span> Conventional System → uses imperative languages, step-by-step instructions (if-then-else, loops). AI-based System → analyzes patterns from data, no predefined rules.</p><p class="text-green-700 text-xs -mt-1">• Thiết kế: Conventional System dùng ngôn ngữ mệnh lệnh, chỉ dẫn từng bước. AI-based System phân tích mẫu từ dữ liệu, không có luật định sẵn.</p>'
    + '<p><span class="text-blue-600 font-medium">• Behavior:</span> Conventional System → deterministic, predictable, transparent. AI-based System → probabilistic reasoning, statistical inference, outputs not always predictable.</p><p class="text-green-700 text-xs -mt-1">• Hành vi: Conventional System xác định, minh bạch, dễ dự đoán. AI-based System dựa trên xác suất, kết quả không phải lúc nào cũng dự đoán trước.</p>'
    + '<p><span class="text-blue-600 font-medium">• Explainability:</span> Conventional System → easy to trace & understand. AI-based System (especially DL) → \"black box\", billions of parameters, difficult to interpret decisions.</p><p class="text-green-700 text-xs -mt-1">• Tính giải thích: Conventional System dễ truy vết. AI-based System (Deep Learning) là \"hộp đen\" với hàng tỷ tham số, khó giải thích.</p>'
    + '<p><span class="text-blue-600 font-medium">• Adaptability:</span> Conventional System → static, requires manual updates. AI-based System → self-learning, continuously improves performance with new data.</p><p class="text-green-700 text-xs -mt-1">• Khả năng thích ứng: Conventional System tĩnh, cần cập nhật thủ công. AI-based System tự học, liên tục cải thiện khi có dữ liệu mới.</p>'
    + '</div>'
  ...
  7: '<div class="mb-2 border-b border-blue-100 pb-1"><span class="text-xs font-bold text-blue-500 uppercase tracking-wider">ENGLISH</span> <span class="text-xs text-blue-400">/</span> <span class="text-xs font-bold text-green-600 uppercase tracking-wider">TIẾNG VIỆT</span></div>'
    ...
    + '<div class="pl-3 mb-3 text-sm space-y-1.5">'
    + '<p><span class="text-blue-600 font-medium">• Wrong API usage →</span> API testing.</p><p class="text-green-700 text-xs -mt-1">• Sai API → API testing.</p>'
    + '<p><span class="text-blue-600 font-medium">• Unsuitable framework →</span> Framework suitability review.</p><p class="text-green-700 text-xs -mt-1">• Framework không phù hợp → Đánh giá tính phù hợp.</p>'
    + '<p><span class="text-blue-600 font-medium">• Broken installation →</span> Smoke testing.</p><p class="text-green-700 text-xs -mt-1">• Cài đặt lỗi → Smoke testing.</p>'
    + '<p><span class="text-blue-600 font-medium">• Library/algorithm bug →</span> ML functional performance testing.</p><p class="text-green-700 text-xs -mt-1">• Thư viện/ thuật toán lỗi → Kiểm thử hiệu suất chức năng ML.</p>'
    + '<p><span class="text-blue-600 font-medium">• Poor hyperparameters →</span> ML functional performance or A/B testing.</p><p class="text-green-700 text-xs -mt-1">• Siêu tham số không tối ưu → A/B testing.</p>'
    + '<p><span class="text-blue-600 font-medium">• Wrong data split →</span> Data allocation review.</p><p class="text-green-700 text-xs -mt-1">• Phân bổ dữ liệu sai → Đánh giá phân bổ.</p>'
    ...
    + '<p><span class="text-blue-600 font-medium">• No improvement over current →</span> Shadow testing.</p><p class="text-green-700 text-xs -mt-1">• Không cải thiện → Shadow testing (kiểm thử bóng).</p>'
    + '</div>'
    + '<div class="mb-2"><strong class="text-blue-700">2. Deployment Testing Types (AI-7.1.2 - K2)</strong></div>'
    + '<div class="pl-3 text-sm space-y-1.5">'
    + '<p><span class="text-blue-600 font-medium">• Installability:</span> Verify MLS can be installed, configured, removed (incl. GPU drivers).</p><p class="text-green-700 text-xs -mt-1">• Kiểm thử cài đặt: Xác minh cài đặt, cấu hình, gỡ bỏ.</p>'
    + '<p><span class="text-blue-600 font-medium">• Rollback:</span> Ability to revert to previous stable state. Must test before production deployment.</p><p class="text-green-700 text-xs -mt-1">• Kiểm thử khôi phục: Quay lại trạng thái ổn định trước đó.</p>'
    + '<p><span class="text-blue-600 font-medium">• Canary:</span> Release to small user group (e.g. 5%), monitor real-time metrics (latency, accuracy).</p><p class="text-green-700 text-xs -mt-1">• Canary: Phát hành cho nhóm nhỏ, theo dõi chỉ số thời gian thực.</p>'
    + '<p><span class="text-blue-600 font-medium">• Shadow:</span> New model runs parallel with current. Both get real requests, only old response goes to users.</p><p class="text-green-700 text-xs -mt-1">• Shadow: Chạy song song, chỉ phản hồi mô hình cũ đến người dùng.</p>'
    + '<p><span class="text-blue-600 font-medium">• Model Conversion:</span> Verify accuracy after format conversion (e.g. training format → mobile format).</p><p class="text-green-700 text-xs -mt-1">• Chuyển đổi mô hình: Kiểm tra độ chính xác sau chuyển đổi.</p>'
    + '<p><span class="text-blue-600 font-medium">• Cross-device:</span> Verify MLS works correctly across all target devices (Cloud → IoT).</p><p class="text-green-700 text-xs -mt-1">• Đa thiết bị: Hoạt động chính xác trên tất cả mục tiêu.</p>'
    + '<p><span class="text-blue-600 font-medium">• API testing:</span> Verify interfaces comply with standards, correct I/O, error messages.</p><p class="text-green-700 text-xs -mt-1">• API: Xác minh giao diện tuân thủ tiêu chuẩn.</p>'
